    ...
    <div ref={ref} className="relative">
      ...
      >
        <div className="hidden sm:flex flex-col items-end">
          ...
          <p className="font-black text-sm group-hover:text-[#ff5833] transition-colors leading-none text-foreground">
      ...
      {/* Dropdown */}
      {open && (
        <div className="absolute right-0 top-[calc(100%+10px)] w-56 bg-card border-3 border-slate-900 dark:border-white/20 rounded-xl shadow-[6px_6px_0px_0px_#23130f] dark:shadow-[6px_6px_0px_0px_rgba(255,255,255,0.1)] z-50 overflow-hidden">
          {/* User info header */}
          <div className="flex items-center gap-3 px-4 py-4 border-b-2 border-slate-100 dark:border-slate-700">
            <div className="size-10 rounded-full bg-[#ff5833] border-2 border-slate-900 dark:border-white/20 flex items-center justify-center overflow-hidden shrink-0">
              {avatar ? (
                <img src={avatar} alt={user.name || user.email} className="w-full h-full object-cover" />
              ) : (
                <span className="text-white font-black text-sm">{initials}</span>
            ...
            <div className="min-w-0">
              <p className="font-black text-sm truncate text-foreground">{user.name || user.email}</p>
          ...
          <div className="py-1.5">
            ...
            <button
              onClick={toggleTheme}
              className="w-full flex items-center gap-3 px-4 py-3 hover:bg-[#ff5833]/5 transition-colors group"
            >
              <span className="material-symbols-outlined text-slate-500 dark:text-slate-400 text-base group-hover:text-[#ff5833]">
